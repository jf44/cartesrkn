<?php
// ce script est un clone de set_positions.php afin de tester en local.
// Ajout d'un formulaire de saisie des données de positions
...
// http://localhost:8080/voilevirtuelle/vgv2020/ge/cartes_rkn/webservice/set_positions.php?boatname=Iroizo C-RKN&course=VGv2020&team=C-RKN&scale=12&nfile=VGv2020_C-RKN_Iroizo C-RKN_20201221_1200.csv
// http://localhost:8080/voilevirtuelle/vgv2020/ge/cartes_rkn/webservice/set_positions.php?boatname=Iroizo C-RKN&course=VGv2020&team=C-RKN&scale=12
// http://localhost:8080/voilevirtuelle/vgv2020/ge/cartes_rkn/webservice/test_vgv2kmz.php?boatname=Iroizo C-RKN&course=VGv2020&team=C-RKN&scale=12
/*
...
$outputdirkmz=$dir_serveur.'/'.$dossier_kmz;
$outputfile="";
$output='';
...
if (!empty($_POST['scale']) && ($_POST['scale']!='') && ($_POST['scale']>0))
{
...
<head>
	<title>VGv2020 : Importation des données de course</title>
...
<body>
<h1>Acquisition et archivage de la position des bateaux sur le Vendée Globe Virtuel 2020</h1>
<h2>Classement de vos Amis et de votre Team</h1>
<p>Installez l\'<a target="_blank" href="https://chrome.google.com/webstore/detail/vr-dashboard/amknkhejaogpekncjekiaolgldbejjan?hl=en">extension Chrome VR Dashboard</a> pour Virtuel Regatta VGV2020.
<br />Puis affichez l\'onglet "<b>FLEET</b>" et sélectionnez les filtres "<i>Team</i>" et "<i>Friends</i>".
Les données affichées ont le format <br />
<code>
RT;Skipper;Last Update;Rank;DTF;DTU;BRG;Sail;State;Position;HDG;TWA;TWS;Speed;Factor;Foils;Hull
<br />#;jf44-RKN;2020-11-11 01:00:00 UTC;473852;22967.72;0.0;-°;Jib (Auto);racing;45°14\'04.74"N 16°46\'27.56"W;179.2;50.6;24.4;12.53;1.0030;?;100%
</code>
<p>Copiez / collez tout le tableau dans le formulaire ci-dessous et cliquez "<b>Valider</b>"
<p>En sortie les caractères, espaces et tabulations inutiles sont purgés, la date (UTC) formatée \'aaaa-mm-dd hh:ii\'
<br />Le classement repose sur la DTU (Distance To User : distance entre votre bateau et celui de votre ami), ce n\'est donc pas celui calculé par Virtual Regatta.
<br />Les données de positions sont archivées dans le dossier <b>'.$outputdir.'</b>
<br />les cartes G.E. le dossier <b>'.$outputdirkmz.'</b><br />
et les classement dans le dossier <b>'.$outputdirclassement.'</b></p>
<form action="'.$appli.'" method="post">
<textarea cols="70" rows="30" name="liste" id="liste">'.$liste.'</textarea>
<input type="hidden" name="boatname" value="'.$boatname.'" />
...
';
	...
	else if (!empty($liste))
	{
		...
		if (!empty($output))
		{
			...
			{
                //echo '<p>DEBUG :: test_vgv2kmz.php :: 207 :: Les données de position sont exportées dans le fichier <code>'.$outputdir.$outputfile.$extension_csv.'</code></p>'."\n";
